"""Fine-tune the HalfKP checkpoint on a mix of the original data (a sample, not
all 75M rows) and the new synthetic "uncompensated material loss" positions, at a
low learning rate for a few epochs -- targeted at the specific gap the autopsy
found, without risking catastrophic forgetting by training on synthetic data alone.

    uv run python finetune_halfkp.py \
        --resume training/ckpt-halfkp-75m-15.pt \
        --synthetic data/synthetic_sacrifice.jsonl \
...
from training.train_halfkp import ACCUMULATOR_NORM, NNUE, remap

SIGMOID_SCALE = 400.0


def pack_jsonl(path: Path, max_rows: int | None = None) -> tuple[np.ndarray, np.ndarray]:
    """Pack a plain fen/cp jsonl file into (idx, target) arrays in memory -- fine
    for the modest row counts this script deals with (real-sample + synthetic),
    unlike pack_halfkp.py's memmap approach built for the full 75M-row set."""
    rows_idx = []
    rows_target = []
    with path.open(encoding="utf-8") as f:
        for line in f:
            ...
            padded = np.full((2, fk.MAX_PIECES), -1, dtype=np.int32)
    ...
    return np.array(rows_idx, dtype=np.int32), np.array(rows_target, dtype=np.float32)


def sample_real_data(data_dir: Path, n: int, seed: int) -> tuple[np.ndarray, np.ndarray]:
    """Random rows from the existing packed HalfKP set -- already in the runtime's
    int32-index format, no re-encoding needed, just a random subset in memory."""
    ...
    ids.sort()  # ascending order turns memmap fancy-indexing into far fewer seeks
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        torch.save(model.state_dict(), checkpoint_dir / f"ckpt-halfkp-{args.tag}-{epoch}.pt")

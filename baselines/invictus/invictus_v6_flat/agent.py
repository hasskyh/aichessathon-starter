...
#
# invictus_v6_flat is this same design, but with the endgame net trained flat
# (train.py --outputs 0: no hidden layer at all, output.weight applied straight to
# the clamped accumulator -- see nnue.forward_flat). That has a genuinely different
# weight shape (w3 is 2*HIDDEN wide instead of OUTPUTS wide, and there is no w2/b2 at
# all), so it cannot reuse negamax/quiescence's signature the way the normal-
# architecture endgame net does -- hence negamax_flat/quiescence_flat below, a
# second, leaner pair used only for the endgame branch, exactly analogous to how
# think() already picks one net or the other once per move.
...
# Flat architecture (train.py --outputs 0), trained only on positions under
# ENDGAME_PIECE_LIMIT pieces: no hidden layer, so only w1/b1 (the accumulator's own
# embedding, same shape as the main net's) and w3/b3 (2*HIDDEN wide here, applied
# straight to the clamped accumulator -- see nnue.forward_flat) exist at all.
...
def nnue_evaluate_flat(
    ...
    """Same contract as nnue_evaluate, for the flat (no hidden layer) net."""
...
def quiescence_flat(
    ...
    """Identical in structure to quiescence above, for the flat endgame net."""
    if _tick(ctrl):
        return 0
    if ply >= MAX_PLY - 2:
        return nnue_evaluate_flat(stack, ply, st, w3, b3)
    ...
    if checkers:
        ...
        best = nnue_evaluate_flat(stack, ply, st, w3, b3)
        if best >= beta or qdepth == 0:
            return best
        if best > alpha:
            alpha = best
    ...
    for i in range(count):
        ...
        stack[ply + 1] = stack[ply]
        nnue.update(stack[ply + 1], w1, off, on)
        score = -quiescence_flat(
            bb, sq, st, -beta, -alpha, qdepth - 1, ply + 1, buf, scores, hist, ctrl,
            stack, w1, w3, b3, butterfly
        )
        unmake_move(bb, sq, st, move, hist, ply)
        if ctrl[2] != 0.0:
            return 0
...
def negamax_flat(
    ...
    """Identical in structure to negamax above, for the flat endgame net."""
    if _tick(ctrl):
        return 0, -1
    if depth == 0:
        return quiescence_flat(
            bb, sq, st, alpha, beta, QUIESCE_DEPTH, ply, buf, scores, hist, ctrl,
            stack, w1, w3, b3, butterfly
        ), -1
    ...
    if not checkers and depth >= 3 and has_non_pawn_material(bb, st[0]):
        ...
        stack[ply + 1] = stack[ply]
        hash_stack[ply + 1] = hash_stack[ply] ^ hash_delta
        child_score, _ = negamax_flat(bb, sq, st, -beta, -beta + 1, depth - 1 - R, ply + 1,
                                 buf, scores, hist, ctrl, stack, w1, w3, b3,
                                 hash_stack, tt_key, tt_move, tt_score, tt_depth, tt_type,
                                 butterfly, game_history, game_history_len
                            )
        score = - child_score
    ...
    for i in range(count):
        ...
        nnue.update(stack[ply + 1], w1, off, on)
        hash_stack[ply + 1] = hash_stack[ply] ^ hash_delta
        child_score, _ = negamax_flat(
            bb, sq, st, -beta, -alpha, depth - 1, ply + 1, buf, scores, hist, ctrl,
            stack, w1, w3, b3, hash_stack,
            tt_key, tt_move, tt_score, tt_depth, tt_type, butterfly, game_history, game_history_len
        )
        score = -child_score
        unmake_move(bb, sq, st, move, hist, ply)
...
def _warm() -> None:
    ...
    negamax_flat/quiescence_flat are separate functions, not reachable from inside
    negamax's own body (unlike invictus_v6's normal-architecture endgame net, which
    reuses negamax/quiescence directly) -- think() calls them straight from plain
    Python, so compiling negamax does not also compile these. They need their own
    explicit warm-up call below, or the first real endgame position in a game would
    pay their compile cost on the clock instead of during the init budget.
    """
    ...
    negamax_flat(
        bb, sq, st, -INF, INF, 1, 1, BUF, SCORES, HIST, CTRL, STACK, W1E, W3E, B3E, HASH_STACK,
        TT_KEY, TT_MOVE, TT_SCORE, TT_DEPTH, TT_TYPE, HISTORY, GAME_HISTORY, GAME_HISTORY_LEN
    )
    unmake_move(bb, sq, st, move, HIST, 0)

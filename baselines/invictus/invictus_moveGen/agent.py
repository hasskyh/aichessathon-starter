...
def think(
    ...
    for depth in range(1, max_depth + 1):
        ...
        for move in [pv] + [m for m in moves if m != pv]:
            ...
            score = -negamax(
                bb, sq, st, -INF, -best_score, depth - 1, 1, buf, scores, hist, ctrl
            )
...
def _warm() -> None:
    ...
    the ordering helpers, and bitgen's generation and make/unmake at the int32 move
    type the search really uses. Argument types must match the real call exactly, or
    numba compiles a second specialisation later, on the clock.
    """
    ...
    make_move(bb, sq, st, move, HIST, 0)
    negamax(bb, sq, st, -INF, INF, 1, 1, BUF, SCORES, HIST, CTRL)
    unmake_move(bb, sq, st, move, HIST, 0)

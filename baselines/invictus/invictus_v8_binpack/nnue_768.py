...
def forward_flat(acc, stm, w3, b3):  # type: ignore[no-untyped-def]
    """Score the position for a flat (no hidden layer) net: output.weight applied
    directly to the clamped accumulator, skipping forward()'s hidden stage entirely.

    w3 here plays the same role -- and uses the same ACT_MAX-scaled quantisation --
    as forward()'s w3, just 2*HIDDEN wide instead of OUTPUTS wide, since there is no
    intermediate stage to shrink that width down first. See export.py's quantize().
    """
    half = acc.shape[1]
    other = 1 - stm

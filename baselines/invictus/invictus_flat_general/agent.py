...
#
# This build uses a flat (no hidden layer) NNUE as its ONLY, general-purpose net --
# not an endgame specialist (see invictus_v6_flat), the whole game (see
# training/train.py --outputs 0, ckpt-flat-general-10.pt). nnue.forward_flat applies
# the output layer straight to the clamped accumulator, skipping the normal
# architecture's 512->32 hidden stage entirely -- motivated by the depth-comparison
# finding that the normal net's per-node cost (that hidden-layer matmul) costs a full
# ply of search depth against material eval at this engine's typical time budget; a
# flat net should recover most of that speed while still contributing a learned eval.
...
# Flat architecture (train.py --outputs 0): only w1/b1 (the accumulator's own
# embedding) and w3/b3 (applied straight to the clamped accumulator, no hidden
# stage) -- no w2/b2 at all. Separately: numba types a module-level global array
# as read-only the moment it is touched inside a jitted function, regardless of the
# array's own .flags.writeable. That is why W1/W3/B3 are threaded through negamax,
...
# whatever memory layout the .npz happened to store; ascontiguousarray forces
# genuine C order regardless of what was on disk.
...
def quiescence(
    ...
    if _tick(ctrl):
        return 0
    if ply >= MAX_PLY - 2:
        return nnue_evaluate(stack, ply, st, w3, b3)
    ...
    if checkers:
        ...
        best = nnue_evaluate(stack, ply, st, w3, b3)
...
def _warm() -> None:
    ...
    HASH_STACK[1] = HASH_STACK[0] ^ hash_delta
    negamax(
        bb, sq, st, -INF, INF, 1, 1, BUF, SCORES, HIST, CTRL, STACK, W1, W3, B3, HASH_STACK,

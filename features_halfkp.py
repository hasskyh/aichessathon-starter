...
def halfkp_deltas(
    ...
    When a perspective needs a refresh, its off/on rows are meaningless -- discard
    them and call halfkp_active() (or the bb equivalent) fresh for that side instead.
    Call this before board.push(move), exactly like features.deltas().
    """
    ...
    if mover.piece_type == chess.KING:
        white_refresh = mover.color == chess.WHITE
        black_refresh = mover.color == chess.BLACK
        empty = (np.zeros((2, 0), dtype=np.int32), np.zeros((2, 0), dtype=np.int32))
        return empty[0], empty[1], white_refresh, black_refresh

    white_king = board.king(chess.WHITE)
    black_king = board.king(chess.BLACK)
    off: list[tuple[chess.Piece, chess.Square]] = [(mover, move.from_square)]
    on: list[tuple[chess.Piece, chess.Square]] = [
        (chess.Piece(move.promotion or mover.piece_type, mover.color), move.to_square)
    ]
    ...
    # Castling changes the king's own square, so it is handled by the king branch
    # above (the rook move alone, without the king move, cannot occur in chess).
    ...
    return encode(off), encode(on), False, False
...
def halfkp_feature_index_bb(king_square: int, code: int, square: int, perspective: int) -> int:
    ...
    a chess.Piece. code // 6 is the colour, code % 6 the piece type; king codes
    (PAWN..KING = 0..5, so code % 6 == bitgen.KING) are never passed in here."""
    colour = code // 6
    piece_type = code % 6
...
def halfkp_deltas_bb(
    ...
    Call this before bitgen.make_move(move), and only for a NON-king move -- a
    king move needs a full halfkp_active_bb() refresh for that perspective instead,
    which the caller (agent.py) decides by checking sq[frm] % 6 == bitgen.KING
